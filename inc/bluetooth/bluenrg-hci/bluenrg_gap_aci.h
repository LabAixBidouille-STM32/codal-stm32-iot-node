 ...
 *       policy set to \93ignore whitelist and process connectable advertising packets only for the
 *       specified device\94. Once a connection is established, GATT procedure is started to read the
 ...
 *      \93use whitelist to determine which advertiser to connect to\94. When a command is issued to
 ...
 *        to \93accept all advertising packets\94 and from the scanning results all the devices
 ...
 *       the controller with the scanner filter policy set to \93accept packets only from
 *       devices in whitelist\94. All the devices found are sent to the upper layer by the
 *      event @cond BLUENRG_MS @ref EVT_LE_ADVERTISING_REPORT.@endcond @cond BLUENRG @ref EVT_BLUE_GAP_DEVICE_FOUND.@endcond
 ...
 * @note  A LE_Create_Connection call will be made to the controller by GAP with the initiator filter
 *       policy set to \93ignore whitelist and process connectable advertising packets only for the
 *       specified device\94. The procedure can be terminated explicitly by the upper layer by issuing
